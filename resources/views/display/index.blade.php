    ...
    <style>
        ...
        *, *::before, *::after {
        ...
        .kiosk-logo {
            ...
            padding: 5px;
            margin-bottom: 10px;
            max-height: 80px; /* menyesuaikan tinggi header */
        }
        
        .kiosk-logo-img {
            max-height: 70px; /* batasi tinggi logo agar serasi */
            width: auto;      /* proporsi otomatis sesuai gambar */
            object-fit: contain;
...
<div class="kiosk-logo">
    @if(isset($pengaturan->logo) && $pengaturan->logo)
        @php
            // Gunakan langsung jika URL Cloudinary
            $logoSrc = (str_starts_with($pengaturan->logo, 'http') || str_starts_with($pengaturan->logo, 'https'))
                ? $pengaturan->logo
...
<script>
    ...
    // ============================================================
    // AUDIO PLAYBACK FUNCTIONS (REVISI: HANYA Web Speech API)
    // ============================================================
    ...
    function updateDisplay() {
        // URL untuk ambil data dari backend (pastikan route ini ada)
        fetch('{{ route("display.data") }}')
            .then(response => response.json())
            .then(data => {
                // ... (Logika Error Handling dan Pemrosesan Data)
