...
<body>
    ...
    <div class="sidebar" id="mySidebar">
        <div class="brand">
            @php
                $setting = \App\Helpers\SettingHelper::get();
                $logo = $setting?->logo;
                $namaInstansi = $setting?->nama_instansi ?? 'Antrian Ruang Coding';
            @endphp
            
            @if($logo)
                @php
                    // Support both absolute URLs (Cloudinary) and local storage paths
                    $logoSrc = (str_starts_with($logo, 'http') || str_starts_with($logo, 'https'))
                        ? $logo
                        : asset('storage/' . $logo);
                @endphp
                <img src="{{ $logoSrc }}" alt="Logo" style="height: 32px; width: auto; object-fit: contain;">
            @else
